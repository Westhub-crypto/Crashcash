...
/* ═══════════════════════════════════════════════════════════
   CRUSHCASH v2  —  FIXED: components outside App (no remount)
═══════════════════════════════════════════════════════════ */
...
// ── Design tokens (module-level — NOT inside any component) ──
const V = {
...
// ── Fixed apiFetch with error handling ────────────────────
const apiFetch = async (method, path, body, token) => {
  try {
    ...
    if (!res.ok && res.status === 0) {
      return { success:false, error:"Cannot connect to server. Please check your internet." };
    }
...
// ══ CANDY SVG DEFS (module-level component) ═══════════════
const CandyDefs = () => (
    ...
    return (
      <div style={{position:"relative",width:s,height:s,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} filter={filt}>
          ...
          <ellipse cx={cx*0.72} cy={cy*0.65} rx={s*0.12} ry={s*0.09} fill="rgba(255,255,255,0.58)"/>
    ...
    return (
      <div style={{position:"relative",width:s,height:s}}>
        <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} filter={filt}>
          ...
          <ellipse cx={cx*0.76} cy={cy*0.78} rx={s*0.14} ry={s*0.19} fill="rgba(255,255,255,0.55)"/>
          {selected&&<path d={d} fill="none" stroke="white" strokeWidth="2.5"/>}
        </svg>
      </div>
    );
  }
  return (
    <div style={{position:"relative",width:s,height:s}}>
      <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} filter={filt}>
        ...
        <ellipse cx={cx*0.63} cy={cy*0.54} rx={r*0.43} ry={r*0.31} fill="rgba(255,255,255,0.60)"/>
...
const BoardCell = ({ children, onClick }) => (
  <div onClick={onClick} style={{width:"52px",height:"52px",borderRadius:"11px",background:"linear-gradient(160deg,#3D1257 0%,#270840 100%)",boxShadow:"inset 0 2px 5px rgba(0,0,0,0.55),inset 0 -1px 2px rgba(255,255,255,0.04)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"transform .1s"}}>
    {children}
  </div>
);

// ══ SHARED UI COMPONENTS (module-level) ═══════════════════
// ⚠ CRITICAL: These MUST be outside App to prevent remounting on every render
...
const Card = ({ children, style={} }) => (
  <div style={{background:V.card,border:`1px solid ${V.border}`,borderRadius:"18px",padding:"22px",...style}}>
    {children}
  </div>
);

// ⚠ Field is the most critical component to keep outside App.
// When defined inside App, every keystroke re-creates Field causing input to lose focus.
const Field = ({ label, value, onChange, placeholder, type="text" }) => {
  const handleFocus = e => { e.target.style.borderColor = V.pri3; };
  const handleBlur  = e => { e.target.style.borderColor = V.border; };
  return (
    <div>
      ...
      <input
        ...
        onFocus={handleFocus}
        onBlur={handleBlur}
        style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${V.border}`,borderRadius:"10px",padding:"12px 15px",color:V.txt,fontSize:"14px",outline:"none",boxSizing:"border-box",transition:"border .2s",fontFamily:"inherit"}}
      />
    </div>
  );
};
...
const Navbar = ({ links=true }) => {
  const { nav, user, bal, doLogout } = useApp();
  return (
      ...
      <div style={{width:"100%",maxWidth:"420px",paddingTop:"64px"}}>
        ...
        <div style={{display:"flex",background:"rgba(255,255,255,0.04)",borderRadius:"12px",padding:"4px",marginBottom:"22px",border:`1px solid ${V.border}`}}>
          {[["login","Sign In"],["register","Register"]].map(([m,l])=>(
            <button key={m} onClick={()=>setAMode(m)} style={{flex:1,padding:"10px",borderRadius:"10px",border:"none",cursor:"pointer",fontWeight:"700",fontSize:"14px",transition:"all .2s",fontFamily:"inherit",background:aMode===m?`linear-gradient(135deg,${V.pri},${V.pri3})`:"transparent",color:aMode===m?"#fff":V.txt2}}>{l}</button>
          ))}
        </div>
        <Card style={{padding:"28px"}}>
          <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
            ...
            {aErr && (
              <div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"9px",padding:"11px 14px",fontSize:"13px",color:V.red}}>⚠ {aErr}</div>
            )}
            {aMode==="register" && (
              <div style={{background:"rgba(16,185,129,0.1)",border:`1px solid rgba(16,185,129,0.25)`,borderRadius:"9px",padding:"10px 14px",fontSize:"13px",color:V.grn}}>
      ...
      <div style={{maxWidth:"1200px",margin:"0 auto",padding:"36px 24px"}}>
        ...
        {user?.kycStatus!=="verified" && (
          <div style={{background:"rgba(245,158,11,0.08)",border:`1px solid rgba(245,158,11,0.25)`,borderRadius:"12px",padding:"14px 18px",marginBottom:"22px",display:"flex",alignItems:"center",gap:"12px",flexWrap:"wrap"}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:"700",fontSize:"14px",color:V.acc}}>KYC Required for Withdrawals</div><div style={{fontSize:"13px",color:V.txt2}}>Verify your NIN to unlock withdrawals. Takes 2 minutes.</div></div>
...
// ══ PAGE: GAME ════════════════════════════════════════════
const GamePage = () => {
  const { nav, user, room, board, score, oppScores, sel, tLeft, gameOn, gameOver, gResult, cdLeft, disqMsg, floatEmojis, emojiCd, sessionId, handleCell, sendEmoji } = useApp();
    ...
    <div style={{minHeight:"100vh",background:"#07071A",paddingTop:"64px",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{background:"rgba(7,7,26,0.97)",borderBottom:`1px solid ${V.border}`,padding:"11px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"12px"}}>
        ...
        <div style={{display:"flex",gap:"8px"}}>
          ...
          {gameOn&&<Btn v="out" onClick={()=>nav("lobby")} style={{padding:"9px 16px",fontSize:"13px"}}>🏳 Give Up</Btn>}
          ...
          {gameOn&&(
            <Card style={{padding:"12px"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:"5px"}}>
                {GAME_EMOJIS.map(e=>(
                  <button key={e} onClick={()=>sendEmoji(e)} style={{fontSize:"20px",background:emojiCd?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.07)",border:`1px solid ${V.border}`,borderRadius:"7px",width:"34px",height:"34px",cursor:emojiCd?"not-allowed":"pointer",transition:"all .15s",opacity:emojiCd?.5:1,fontFamily:"inherit"}}>{e}</button>
                ))}
              </div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"14px"}}>
          ...
          {gameOver&&gResult&&!gResult.disqualified&&(
            <div style={{width:"100%",background:gResult.won?"rgba(16,185,129,0.1)":"rgba(239,68,68,0.08)",border:`1px solid ${gResult.won?V.grn:V.red}44`,borderRadius:"16px",padding:"24px",textAlign:"center"}}>
              ...
              <div style={{fontSize:"24px",fontWeight:"900",color:gResult.won?V.grn:V.red,marginBottom:"8px",letterSpacing:"-0.5px"}}>{gResult.won?`You Won ${fmt(gResult.prize)}! 🎉`:"Better Luck Next Time!"}</div>
          ...
          {gameOn&&<div style={{fontSize:"12px",color:V.txt3,letterSpacing:"0.5px"}}>Tap a candy → tap adjacent to swap · Make 3+ in a row to score</div>}
        </div>
      </div>
    </div>
  );
      ...
      <div style={{maxWidth:"820px",margin:"0 auto",padding:"36px 24px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"20px",marginBottom:"22px"}}>
          ...
          <Card>
            ...
            <Btn v="out" onClick={doWithdraw} style={{width:"100%",padding:"13px",borderRadius:"12px",marginTop:"12px"}}>🏦 Withdraw</Btn>
          ...
          {txns.length===0?<div style={{color:V.txt3,textAlign:"center",padding:"30px",fontSize:"14px"}}>No transactions yet</div>:txns.map(t=>(
            <div key={t._id||t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 0",borderBottom:`1px solid ${V.border}`}}>
              <div style={{display:"flex",alignItems:"center",gap:"13px"}}>
                ...
                <div><div style={{fontSize:"13px",fontWeight:"600"}}>{t.description}</div><div style={{fontSize:"11px",color:V.txt3,marginTop:"2px"}}>{(t.date||t.createdAt||"").toString().split("T")[0]}</div></div>
              </div>
  ...
  return (
    <div style={{minHeight:"100vh",paddingTop:"64px",background:V.bg}}>
      ...
      <div style={{maxWidth:"500px",margin:"0 auto",padding:"36px 24px"}}>
        <div style={{textAlign:"center",marginBottom:"32px"}}><div style={{fontSize:"52px",marginBottom:"12px"}}>🔐</div><h1 style={{fontSize:"28px",fontWeight:"900",marginBottom:"6px",letterSpacing:"-0.8px"}}>KYC Verification</h1><p style={{color:V.txt2,fontSize:"14px",lineHeight:1.7}}>Verify your NIN to unlock withdrawals. Required by Nigerian law.</p></div>
        {user?.kycStatus==="verified"?(
          ...
          <Card style={{padding:"28px"}}>
            <div style={{display:"flex",flexDirection:"column",gap:"18px"}}>
              <div style={{background:"rgba(109,40,217,0.08)",border:`1px solid rgba(109,40,217,0.2)`,borderRadius:"10px",padding:"14px",fontSize:"13px",color:V.txt2,lineHeight:1.7}}>
                📋 <strong style={{color:V.txt}}>What is NIN?</strong> Your 11-digit National Identification Number from NIMC. Found on your National ID card, voter card, or NIMC slip.
              </div>
              ...
              <div style={{fontSize:"12px",color:V.txt3}}>🔒 Stored securely. We only verify identity — your NIN is never shared.</div>
  ...
  return (
    <div style={{minHeight:"100vh",paddingTop:"64px",background:V.bg}}>
      ...
      <div style={{maxWidth:"640px",margin:"0 auto",padding:"36px 24px"}}>
        <div style={{textAlign:"center",marginBottom:"32px"}}><div style={{fontSize:"52px",marginBottom:"12px"}}>🎁</div><h1 style={{fontSize:"28px",fontWeight:"900",marginBottom:"6px",letterSpacing:"-0.8px"}}>Refer & Earn</h1><p style={{color:V.txt2,fontSize:"14px",lineHeight:1.7}}>Share your code. Earn <strong style={{color:V.acc}}>₦50</strong> when your friend plays their first game!</p></div>
        <Card style={{marginBottom:"20px",padding:"26px"}}>
          ...
          <div style={{display:"flex",gap:"8px"}}>
            ...
            <Btn onClick={()=>{navigator.clipboard.writeText(link);alert("Copied!");}} style={{padding:"10px 16px",fontSize:"13px",flexShrink:0}}>Copy</Btn>
          </div>
        </Card>
...
const AdminLoginPage = () => {
  const { nav, aLE, setALE, aLP, setALP, adminErr, doAdminLogin } = useApp();
  return (
      ...
      <div style={{flex:1,overflow:"auto",padding:"32px"}}>
        ...
        {(adminTab==="rooms"||adminTab==="transactions"||adminTab==="fraud")&&(
          <div style={{textAlign:"center",paddingTop:"60px"}}>
            ...
            <div style={{fontWeight:"800",fontSize:"18px",marginBottom:"8px"}}>Live MongoDB Data</div>
            <div style={{color:V.txt2,fontSize:"14px",marginBottom:"20px"}}>This section loads live data from your backend API.</div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    s.on("game_start",      d=>{ setBoard(mkBoard()); setScore(0); scoreRef.current=0; setSel(null); setTLeft(GT); setGameOn(true); setGameOver(false); setGResult(null); setDisqMsg(""); startTimer(); });
  ...
  const nav = useCallback(p=>setPg(p),[]);
  ...
  const doAuth = async () => {
    ...
    try {
      if (aMode==="login") {
        ...
        setUser(d.user); setToken(d.token); setBal(d.user.balance||0);
        nav("lobby");
      } else {
        if (!aName||!aEmail||!aPass){ setAErr("Please fill all fields"); return; }
        if (aPass.length<6){ setAErr("Password must be 6+ characters"); return; }
  ...
  const enterRoom = r => {
    ...
    setRoom(r); setBoard(mkBoard()); setScore(0); scoreRef.current=0; setSel(null); setTLeft(GT);
    setGameOn(false); setGameOver(false); setGResult(null); setDisqMsg(""); setOppScores([]); setSessionId(null); setFloatEmojis([]);
  ...
  const doWithdraw = async () => {
    ...
    if(user?.kycStatus!=="verified"){ showWMsg("❌ KYC required first."); nav("kyc"); return; }
  ...
  // Admin
  const doAdminLogin = () => {
    if(aLE===ADMIN_CREDS.email&&aLP===ADMIN_CREDS.pass){ setAdminIn(true); setAdminErr(""); nav("admin"); }
    else setAdminErr("Invalid credentials");
  };

  // Context value — all state & handlers for child components
  const ctxValue = {
    pg, nav, user, setUser, token, bal, setBal,
